    ...
    for d = 1:length(p2c_den.density)
        if p2c_den.density(d) == 384
            exec(conn, sprintf(['insert into %s ',...
                'select pos from %s ',...
                'where density = 384 ',...
                'and (plate_row in (1,16) or plate_col in (1,24))'],...
                tablename_bpos, tablename_p2c));
        elseif p2c_den.density(d) == 1536
            exec(conn, sprintf(['insert into %s ',...
                'select pos from %s ',...
                'where density = 1536 ',...
                'and (plate_row in (1,2,31,32) or plate_col in (1,2,47,48))'],...
                tablename_bpos, tablename_p2c));
        elseif p2c_den.density(d) == 6144
            exec(conn, sprintf(['insert into %s ',...
                'select pos from %s ',...
                'where density = 6144 ',...
                'and (plate_row in (1,2,3,4,61,62,63,64) or ',...
                'plate_col in (1,2,3,4,93,94,95,96))'],...
                tablename_bpos, tablename_p2c));
        else
