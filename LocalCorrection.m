    ...
    for pl = unique(p2c.plate)
        ...
        for c = sort(unique(temp.col))'
            for r = sort(unique(temp.row(temp.col == c)))'
                ...
                neigh(i,1) = isnotempty(temp.pos(temp.col == c - 1 & temp.row == r - 1));
                neigh(i,2) = isnotempty(temp.pos(temp.col == c - 1 & temp.row == r));
                neigh(i,3) = isnotempty(temp.pos(temp.col == c - 1 & temp.row == r + 1));
                neigh(i,4) = isnotempty(temp.pos(temp.col == c & temp.row == r - 1));
                neigh(i,5) = isnotempty(temp.pos(temp.col == c & temp.row == r + 1));
